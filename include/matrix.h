...
static inline struct mat2 mat2_zero()
{
	...
	tmp.x = (struct vec2) { 0, 0 };
	tmp.y = (struct vec2) { 0, 0 };
...
static inline struct mat2 mat2_rotate(float angle)
{
	...
	tmp.x = (struct vec2) { cos(angle), -sin(angle) };
	tmp.y = (struct vec2) { sin(angle), cos(angle) };
...
static inline struct mat3 mat3_zero()
{
	...
	tmp.x = (struct vec3) { 0, 0, 0 };
	tmp.y = (struct vec3) { 0, 0, 0 };
	tmp.z = (struct vec3) { 0, 0, 0 };
...
static inline struct mat3 mat3_rotate_x(float angle)
{
	...
	tmp.x.x = 1.0; tmp.x.y = 0.0; 	tmp.x.z = 0.0;
	tmp.y.x = 0.0; tmp.y.y = cos(angle);  tmp.y.z = sin(angle);
	tmp.z.x = 0.0; tmp.z.y = -sin(angle); tmp.z.z = cos(angle);
...
static inline struct mat3 mat3_rotate_y(float angle)
{
	...
	tmp.x.x = cos(angle);  tmp.x.y = 0.0; tmp.x.z = -sin(angle);
	tmp.y.x = 0.0; 	tmp.y.y = 1.0; tmp.y.z = 0.0;
	tmp.z.x = sin(angle);  tmp.z.y = 0.0; tmp.z.z = cos(angle);
...
static inline struct mat3 mat3_rotate_z(float angle)
{
	...
	tmp.x.x = cos(angle);   tmp.x.y = sin(angle);   tmp.x.z = 0.0;
	tmp.y.x = -sin(angle);  tmp.y.y = cos(angle);   tmp.y.z = 0.0f;
	tmp.z.x = 0.0;		tmp.z.y = 0.0;		tmp.z.z = 1.0;
...
static inline float mat3_determinant(struct mat3 *mat)
{
	return mat->x.x * mat3_minor(mat, 0, 0) -
	       mat->x.y * mat3_minor(mat, 1, 0) +
	       mat->x.z * mat3_minor(mat, 2, 0);
}
...
static inline struct mat4 mat4_zero()
{
	...
	tmp.x = (struct vec4) { 0, 0, 0, 0 };
	tmp.y = (struct vec4) { 0, 0, 0, 0 };
	tmp.z = (struct vec4) { 0, 0, 0, 0 };
	tmp.w = (struct vec4) { 0, 0, 0, 0 };
...
static inline struct mat4 mat4_multf(struct mat4 *mat, float s)
{
	...
	tmp.x.x *= s; tmp.x.y *= s; tmp.x.z *= s; tmp.x.w *= s;
	tmp.y.x *= s; tmp.y.y *= s; tmp.y.z *= s; tmp.y.w *= s;
	tmp.z.x *= s; tmp.z.y *= s; tmp.z.z *= s; tmp.z.w *= s;
	tmp.w.x *= s; tmp.w.y *= s; tmp.w.z *= s; tmp.w.w *= s;
...
static inline struct mat4 mat4_identity()
{
	...
	tmp.x = (struct vec4) { 1, 0, 0, 0 };
	tmp.y = (struct vec4) { 0, 1, 0, 0 };
	tmp.z = (struct vec4) { 0, 0, 1, 0 };
	tmp.w = (struct vec4) { 0, 0, 0, 1 };
...
static inline struct mat4 mat4_translate_vec3(struct vec3 *v)
{
	...
	tmp.y = (struct vec4) { 0, 1, 0, 0 };
	tmp.z = (struct vec4) { 0, 0, 1, 0 };
	tmp.w = (struct vec4) { v->x, v->y, v->z, 1 };
...
static inline struct mat4 mat4_rotate(float angle, float x, float y, float z)
{
	...
	tmp.x = (struct vec4) {   x * x * (1 - cos(angle)) + cos(angle),
				   x * y * (1 - cos(angle)) + z * sin(angle),
				   x * z * (1 - cos(angle)) - y * sin(angle),
				   0 };

	tmp.y = (struct vec4) {   x * y * (1 - cos(angle)) - z * sin(angle),
				   y * y * (1 - cos(angle)) + cos(angle),
				   z * y * (1 - cos(angle)) + x * sin(angle),
				   0 };
	
	tmp.z = (struct vec4) {   x * z * (1 - cos(angle)) + y * sin(angle),
				   y * z * (1 - cos(angle)) - x * sin(angle),
				   z * z * (1 - cos(angle)) + cos(angle),
				   0 };
...
static inline float mat4_determinant(struct mat4 *mat)
{
	return mat->x.x * mat4_minor(mat, 0, 0) -
	       ...
	       mat->x.z * mat4_minor(mat, 2, 0) -
	       mat->x.w * mat4_minor(mat, 3, 0);
}
	...
	float z_near, float z_far)
{
	...
	mat->x = (struct vec4) { zoom_x, 0, 0, 0 };
	mat->y = (struct vec4) { 0, zoom_y, 0, 0 };
	mat->z = (struct vec4) { 0, 0, e, 0 };
	mat->w = (struct vec4) { 0, 0, d, 1 };
}
	...
	float aspect_ratio, float z_near, float z_far)
{
	...
	mat->x = (struct vec4) { zoom, 0, 0, 0 };
	mat->y = (struct vec4) { 0, zoom * aspect_ratio, 0, 0 };
	mat->z = (struct vec4) { 0, 0, d, -1 };
	mat->w = (struct vec4) { 0, 0, e, 0 };
}
	...
	struct vec3 *up)
{
	...
	orientation.x = (struct vec4) { xdir.x, xdir.y, xdir.z, 0 };
	orientation.y = (struct vec4) { ydir.x, ydir.y, ydir.z, 0 };
	orientation.z = (struct vec4) { zdir.x, zdir.y, zdir.z, 0 };
	orientation.w = (struct vec4) { 0, 0, 0, 1 };

	struct mat4 translation;
	translation.x = (struct vec4) { 1, 0, 0, 0 };
	translation.y = (struct vec4) { 0, 1, 0, 0 };
	translation.z = (struct vec4) { 0, 0, 1, 0 };
	translation.w = (struct vec4) { -pos->x, -pos->y, -pos->z, 1 };
